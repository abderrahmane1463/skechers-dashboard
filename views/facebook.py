...
"""
    ...
    with tab4:
        if posts:
            ...
            _type_data = _col.defaultdict(lambda: {"reach": [], "interactions": [], "count": 0})
            for p in posts:
                t = p.get("media_type", "Autre")
                _type_data[t]["reach"].append(p.get("reach", 0))
                _type_data[t]["interactions"].append(p.get("total_interactions", 0))
                _type_data[t]["count"] += 1

            if _type_data:
                _types      = list(_type_data.keys())
                _avg_reach  = [round(sum(v["reach"]) / len(v["reach"])) for v in _type_data.values()]
                ...
                _avg_eng    = [
                    round(sum(v["interactions"]) / sum(v["reach"]) * 100, 2)
                    if sum(v["reach"]) > 0 else 0.0
                    for v in _type_data.values()
                ]

                _type_colors = {"Photo": "#6366f1", "Vidéo / Reel": "#f43f5e",
                                "Carrousel": "#f59e0b", "Lien": "#10b981", "Autre": "#71717a"}

                fig_type = go.Figure()
                fig_type.add_trace(go.Bar(
                    name="Reach moyen",
                    x=_types, y=_avg_reach,
                ...
                for col_w, t, cnt, ar, ai, ae in zip(_cols_list, _types, _counts, _avg_reach, _avg_inter, _avg_eng):
                    ...
                    col_w.markdown(
                        ...
                        f'<div style="font-size:0.68rem;color:{_lc};margin-top:0.3rem;">Reach moyen</div>'
                        f'<div style="font-size:0.95rem;font-weight:700;color:{_vc};">{ar:,}</div>'
