...
| "Result type" / "Results" columns | The unified Meta "Results" metric is split into two columns. `api/boost.py:_derive_result()` maps each campaign objective to its result kind: Sales→Purchases, Awareness/Reach→Reach, Engagement→Post engagement, Traffic→Link clicks, Leads→Leads, Video→Video views, Messages→Messaging conversations. `result_type`, `results`, and `cost_per_result` are set on both ad rows and campaign rows. Requires a Refresh to repopulate cached rows that predate this. |
| Rapport tab = two tables | **📋 Rapport Hebdomadaire Sales** is ad-level (built from `adset_ad_data["ads"]`, 43 cols, default filter `OUTCOME_SALES`, multi-row select). **📊 Rapport Hebdomadaire Notoriété & Engagement** is **campaign-level** (built from `boost_data["campaigns"]`, one row per campaign, 19 cols, default filter all-except-`OUTCOME_SALES`). Campaign-level is used so reach/results are Meta-deduplicated and match Ads Manager — an ad-level view duplicated each campaign once per ad. `_build_campaign_row()` in `views/boost.py`. |
| Dashboard showed fewer rows/lower numbers than Ads Manager | Three causes, all fixed: (1) **No pagination** — campaign/adset/ad insights and the `/campaigns` & `/adsets` meta calls only read the first 500-row page; now all use `_get_ads_all()` which follows Meta's `paging.next` URLs to the end. (2) **Partial ID scans were persisted** — `_get_skechers_ids()` saved an interrupted (truncated) campaign-ID list to Supabase for 24h; now only a *complete* scan (`status["complete"]`) is persisted, partials are used for the render but expire in 1h and are never saved. (3) **Attribution mismatch** — insights now send `action_attribution_windows=["7d_click","1d_view"]` to match Ads Manager's default, so conversion counts line up. `_get_ads_all()` also returns rows already collected if a later page errors (rate limit), instead of discarding everything. |
| `/campaigns` 400 on id-filter or effective_status | The `/campaigns` edge rejects both `filtering` by `id` and a JSON `effective_status` param (tested → HTTP 400). Meta budget/status/date meta is therefore fetched unfiltered with full pagination, then filtered client-side by Skechers ID set. Do not re-add those params. |
| Skechers campaign-ID cache never persisted → constant rate-limiting | `metric_cache.period_start`/`period_end` are SQL `date` columns. The IDs row used the literal `"static"` as its period key, so every save raised Postgres `22007 invalid input syntax for type date` — but `_supabase_save_ids()` didn't check the response, so it printed "saved" while persisting nothing. Tier 2 always missed → every fetch ran the CPU-heavy full `/ads` scan → Marketing API `total_cputime` rate limit (80004). Fixed: IDs row uses date sentinel `_IDS_PERIOD = "2000-01-01"`, and the save now logs real HTTP failures. |
